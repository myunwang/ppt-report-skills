...
**Human silhouette portrait** (`human-portrait` template — consulting-grade businessman silhouette with leader-line anchors to surrounding labels; born from "OpenAI IPO" deck, OpenClipart CC0):

![Human portrait](docs/screenshots/human-portrait.png)

> Central silhouette stays clean — colored dots on the silhouette mark body parts, dashed leader lines connect to labels on the slide edges. No background highlight discs. Works for user persona, anthropomorphic capability maps, job-risk heatmaps, and team pictograms.
...
- **8 page templates** (`assets/slides-templates/`):
  ...
  - `landscape-map` — industry / competitor landscape (12-grid 3-tier nested + logo wall; pure CSS, no ECharts)
  - `matrix-2x2` — **NEW** McKinsey / BCG-style 2×2 strategic matrix (4 quadrants + bubble positioning + 3 strategic insights; pure CSS, not ECharts scatter)
  - `human-portrait` — **NEW ⭐ consulting-grade human silhouette portraits** (male businessman with briefcase + female long-haired professional, OpenClipart CC0; central silhouette + 5 surrounding labels + leader lines + colored dot anchors). Use for **user persona / anthropomorphic capability map / job-risk heatmap / team pictogram**. Pre-baked into `common.js` as `window.SILHOUETTE_DATA + injectSilhouette(svg, 'man'|'woman')` — drop-in shared across all slides
...
- **9 reference docs** (`references/`, ~2000 lines): architecture · design system · layout principles · chart selection · components · themes · landscape-qa · landscape-skeleton · **matrix-2x2** · **svg-aesthetics** · **human-portrait**
...
**人体剪影画像**（`human-portrait` 模板 — 咨询报告级西装提包剪影 + 引线锚定周边标签；源自"OpenAI IPO"deck,OpenClipart CC0）：

![人体剪影](docs/screenshots/human-portrait.png)

> 中央剪影保持干净 — 剪影身上以小色点标识部位,虚线引线连接到 slide 边缘的标签卡片。**不用背景高亮圆**。适合用户画像、拟人化能力地图、职业风险热图、团队 pictogram。
...
- **8 套页面模板**（`assets/slides-templates/`）：
  - `kpi-overview` — 多板块 × 多卡片总览
  - `two-country` — 双对象左右对照（KPI + metric 表）
  - `three-phase` — 三阶段时间线 + 多图表
  - `multi-trend` — 多对象趋势对比
  - `supply-bars` — 分类条形 + 趋势折线
  - `landscape-map` — 产业图谱 / 竞品全景（12 栅格 3-tier 嵌套 + logo 墙；纯 CSS）
  - `matrix-2x2` — **新增** 麦肯锡 / BCG 风 2×2 战略矩阵（4 象限 + 气泡定位 + 3 条战略洞察；纯 CSS,不用 ECharts scatter）
  - `human-portrait` — **新增 ⭐ 咨询报告级人体剪影画像**（西装提包男 + 长发耸肩职业女,OpenClipart CC0;中央剪影 + 5 个标签卡环绕 + 引线 + 小色点锚定部位;**无背景高亮圆**保持剪影干净）。适用 **用户画像 / 拟人化能力地图 / 职业风险图 / 团队 pictogram**。剪影 path 预置在 `common.js` 的 `window.SILHOUETTE_DATA + injectSilhouette(svg, 'man'|'woman')`,所有 slide 调用一份即可
- **5 套预设主题**：modern-light / dark-tech / warm-business / brand-blue / minimal-mono（右上角一键切，localStorage 持久化）
- **9 份参考文档**（`references/`，~2000 行）：架构 · 设计系统 · 布局法则 · 图表选型 · 组件 · 主题 · landscape-qa · landscape-skeleton · **matrix-2x2** · **svg-aesthetics** · **human-portrait**
