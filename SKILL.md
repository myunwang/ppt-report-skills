...
| 人体画像 / 拟人化能力 / 风险热图 / 团队 pictogram | `human-portrait.html` | **真正的异形容器页型 ⭐ 难骨头**。中央 SVG 人体 + 部位高亮 + 周边标签错落 + 引线连接。典型场景:用户画像 / 模型能力拟人 / 职业风险热图 / 团队人数 pictogram。**人体 SVG 直接抄 OpenMoji 不要手画**(详见 `references/svg-aesthetics.md`),所有坐标必须用 OpenMoji 原 72×72 系防止部位高亮错位。详见 [`references/human-portrait.md`](references/human-portrait.md) — 4 种变体 + 关键部位坐标表 + 7 条踩坑黑名单 |
...
- [`references/human-portrait.md`](references/human-portrait.md) — **人体画像页型**(异形容器类难骨头)。中央 SVG 人体 + 部位高亮 + 周边标签 + 引线连接。4 种变体 + 关键部位坐标表 + 7 条踩坑黑名单。配套 `human-portrait.html` 模板
